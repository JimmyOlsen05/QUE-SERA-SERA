...
export default function GroupChat({ groupId }: GroupChatProps) {
  ...
  useEffect(() => {
    ...
    // Subscribe to real-time notifications for new messages
    const subscription = supabase
      ...
      .on(
        ...
        async (payload) => {
          ...
          if (groupData && senderData) {
            const notification = {
              user_id: user.id,
              title: `New message in ${groupData.name}`,
              content: `${senderData.username}: ${payload.new.content.substring(0, 50)}${payload.new.content.length > 50 ? '...' : ''}`,
              type: 'group_message',
              read: false,
              metadata: {
                group_id: groupId,
